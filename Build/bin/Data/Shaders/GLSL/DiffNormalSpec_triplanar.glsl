...
void PS()
{
    vec3 weights=abs(vNormal);  // calculate the weights of the three projected textures
    ...
    vWorldPos_currected*=0.0625;
        ...
        vec4 diffColor=texture2D(sDiffMap,vWorldPos_currected.xy)*weights.z+  // project each of the three planes and blend together with the weights
                       texture2D(sDiffMap,vWorldPos_currected.yz)*weights.x+
                       texture2D(sDiffMap,vWorldPos_currected.xz)*weights.y;
    ...
    #ifdef SPECMAP
        vec3 specColor=texture2D(sSpecMap,vWorldPos_currected.xy).rgb*weights.z+  // project each of the three planes and blend together with the weights
                       texture2D(sSpecMap,vWorldPos_currected.yz).rgb*weights.x+
                       texture2D(sSpecMap,vWorldPos_currected.xz).rgb*weights.y;
    ...
    // Get fog factor
        ...
        #endif
        #ifdef EMISSIVEMAP
            finalColor += cMatEmissiveColor * texture2D(sEmissiveMap, tex_coord_top).rgb;
        #else
            finalColor += cMatEmissiveColor;
        #endif
